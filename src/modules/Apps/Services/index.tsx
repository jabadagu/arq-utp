...
import { useFetchServices } from "./hooks/useServiceList";
...
const Services: React.FC = () => {
  const { data, loading } = useFetchServices();
  ...
  const filteredData = useMemo(() => {
    ...
    return list.filter((row: any) => {
      ...
      const categoriaId = String(row.categoriaId ?? "").toLowerCase();
      const precioBase = String(row.precioBase ?? "").toLowerCase();
      return (
        nombre.includes(q) ||
        descripcion.includes(q) ||
        id.includes(q) ||
        categoriaId.includes(q) ||
        precioBase.includes(q)
      );
    });
  ...
  return (
    <>
      ...
      <ModalForm
        ...
        texts={{ title: CONFIRMS.deleteService }}></ModalForm>
    </>
  );
};
